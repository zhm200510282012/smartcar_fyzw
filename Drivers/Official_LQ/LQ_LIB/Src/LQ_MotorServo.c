 ...
 * 最后更新：2026年1月17日
 * 软件版本：V1.0
 * 调用举例：BLmotor_Init_(50);  //初始化一体板上P23引脚为PWM，频率50Hz,满占空比20000
 ******************************************************************************************/
void BLmotor_Init_1(u16 freq)
{
    ...
    PWMx_InitStructure.PWM_Mode = CCMRn_PWM_MODE1; // 模式,
    PWMx_InitStructure.PWM_Duty = 0;               // PWM占空比时间, 0~Period
    PWMx_InitStructure.PWM_EnoSelect = ENO2N;      // 输出通道选择 PWM3_N通道 P23
    PWM_Configuration(PWM2, &PWMx_InitStructure);  // 初始化PWM3,  PWMA
...
/******************************************************************************************
 * 函 数 名：void BLmotor_Ctrl_w1(u16 BLmotor1,u16 BLmotor2)
 * 功    能：BLmotor 无感无刷电调控制程序，当PWM高电平时间在1-2ms区间内，无刷电机旋转，2ms时转速最高
 * 参 数 值：u16 BLmotor1，占空比，范围0--10000；
 * 作    者：LQ008
 * 最后更新：2026年1月17日
 * 软件版本：V1.0
 * BLmotor_Ctrl_w1(1000);//当频率为50时，设置高电平时间为1ms
 ******************************************************************************************/
void BLmotor_Ctrl_w1(u16 BLmotor_duty)
{
    if (BLmotor_duty <= 900) // 限制幅值,根据实际情况进行调整
    {
        BLmotor_duty = 900;
    }
    if (BLmotor_duty >= 2000) // 限制幅值,根据实际情况进行调整
    {
        BLmotor_duty = 2000;
    }
