...
static s16 clamp_offset(s16 value, s16 limit)
{
    if (limit < 0) {
        limit = (s16)-limit;
    }
...
{
    return track_strategy_mode_params(mode);
}
                               ...
                               const ctrl_fuzzy_steering_input_t *input)
{
    ...
    params = ctrl_fuzzy_steering_params(input->mode);

    if (ctrl_fuzzy_steering_needs_reset(input->app_state,
                                        input->mode,
                                        input->signal_quality,
                                        input->outputs_allowed) != APP_FALSE) {
        ctrl_fuzzy_steering_reset(state);
    ...
    limited = clamp_offset((s16)output, params->steering_offset_limit_us);
    limited = apply_rate_limit(limited,
                               state->previous_output_us,
                               params->steering_rate_limit_us_per_tick);
    limited = clamp_offset(limited, params->steering_offset_limit_us);

    state->previous_error = input->line_error_filtered;
    state->previous_output_us = limited;
    return limited;
}
